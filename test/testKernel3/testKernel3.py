...
inputCellType = sim.SpikeSourceArray("input", nExc, "EXCITATORY_NEURON", "CUBA")
...
# create neuron groups. 
izhikevichCellType2 = sim.Izhikevich("INHIBITORY_NEURON", a=0.1, b=0.2, c=-65, d=2, i_offset=[0.014, 0.0, 0.0])
...
connection = sim.Projection(neuron_group1, neuron_group1, sim.FixedProbabilityConnector(pConn), receptor_type='excitatory')

# connect input to exc - "random connector"
connection = sim.Projection(neuron_group1, neuron_group2, sim.FixedProbabilityConnector(pConn), receptor_type='inhibitory')

# connect input to exc - "random connector"
connection = sim.Projection(neuron_group2, neuron_group1, sim.FixedProbabilityConnector(pConn), receptor_type='inhibitory')


# set STDP for all incoming synapses to 'exc' group
alphaPlus = 0.1
tauPlus = 20.0
alphaMinus = 0.1
tauMinus = 20.0

sim.state.network.setESTDP(1, bool(1), STANDARD, ExpCurve(alphaPlus, tauPlus, -alphaMinus, tauMinus))
sim.state.network.setISTDP(2, bool(1), STANDARD, ExpCurve(-alphaPlus, tauPlus, alphaMinus, tauMinus))
